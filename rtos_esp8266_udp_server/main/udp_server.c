...
static void i2c_task()
{
  ...
  while(1){
    ...
    if ((ret == ESP_OK) && (ret1 == ESP_OK)) {
      ...
      xQueueSend(buffer_queue, &buffer, pdMS_TO_TICKS(0));
      vTaskDelay(100 / portTICK_RATE_MS);
    } 

    else {
      ESP_LOGE(TAG, "No ack, sensor %s not connected...skip...\n", ret == ESP_OK ? "Posição" : ret1 == ESP_OK ? "Referência" : "None");
    }
    ...
    ESP_LOGI(TAG, "Waiting for address");
          ...
          // Data received
            ...
            while (1) {
              ...
              if(xQueueReceive(buffer_queue, &tx_buffer, pdMS_TO_TICKS(1000))==true){

                sprintf(tx_buffer_msg,  "x_ref:%d y_ref:%d z_ref:%d\tx_pos:%d y_pos:%d z_pos:%d\n",
                  tx_buffer[0],
                  tx_buffer[1],
                  tx_buffer[2],
                  tx_buffer[3],
                ...
                int err = sendto(sock, tx_buffer_msg, sizeof(tx_buffer_msg), 0, (struct sockaddr *)&sourceAddr, sizeof(sourceAddr));

                if (err < 0) {
                  ESP_LOGE(TAG, "Error occured during sending: errno %d", errno);
                  break;
                }
                ESP_LOGI(TAG,"nº of characters: %d", err);
...
void app_main()
{
  ...
  buffer_queue = xQueueCreate(24, 6*sizeof(int16_t));//Cria a queue *buffer* com 10 slots de 4 Bytes
