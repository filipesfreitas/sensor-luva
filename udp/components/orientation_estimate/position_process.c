...
void orientation_estimation(raw_data metacarpo,raw_data proximal,Glove* glove,int i) {
	...
	if (proximal.accelx + proximal.accely + proximal.accelz == 0) {
		...
		acos(proximal.accelz/sqrt(proximal.accelx*proximal.accelx + proximal.accely*proximal.accely+proximal.accelz*proximal.accelz ))*degre_conv*(1-alpha) +
...
float fix_orientation(raw_data raw,float angle){
	...
	if (raw.accelz>=0 && raw.accelx<0 	) angle += 360; 
	if (raw.accelz<0 	&& raw.accelx>=0 	) angle = 180-angle;
	if (raw.accelz<0 	&& raw.accelx<0 	) angle += 180;
	return angle;
}
...
void reference_frame_orientation(raw_data reference,Glove* glove){
	/* PHI	*/
	if (reference.accelx + reference.accely + reference.accelz == 0) {

	}else{
		glove->frame_reference.phi = acos(reference.accelz/sqrt(reference.accelx*reference.accelx + 
			reference.accelz*reference.accelz + reference.accely*reference.accely ))*
		degre_conv*(1-alpha) +	(glove->frame_reference.phi + reference.gyroy * period ) * alpha ;
	}
	/* THETA */
	if (reference.accelx==0) glove->frame_reference.theta = 0;
	else glove->frame_reference.theta = atan(reference.accely/reference.accelx)*degre_conv*(1-alpha) +
...
int buffer_arrange(Glove* glove, char message[]){
	...
	int length_message = sprintf(message,"%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\r\n",
		...
		glove->fingers[1].proximal.phi-glove->fingers[1].metacarpo.phi -	glove->frame_reference.phi,
		glove->fingers[1].pressure,
		glove->fingers[2].metacarpo.theta,
		glove->fingers[2].metacarpo.phi -	glove->frame_reference.phi,
		glove->fingers[2].proximal.theta,
		glove->fingers[2].proximal.phi-glove->fingers[2].metacarpo.phi -	glove->frame_reference.phi,
		glove->fingers[2].pressure,
		glove->fingers[3].metacarpo.theta,
		glove->fingers[3].metacarpo.phi -	glove->frame_reference.phi,
